...
public class ConfigScreen extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (section.calculateSectionHeight() > 0) {
            ...
            context.fill(x, y, x + 6, y + height, 0xFFAAAAAA); // Простой серый скроллбар
        }
    ...
    private void drawDirtTextureBlurred(DrawContext context, int x, int y, int width, int height) {
        int color = ColorHelper.Argb.fromFloats(.4f, 0, 0, 0);
        context.setShaderColor(0.5f, 0.5f, 0.5f, 1.0f);
        context.drawTexture(OPTIONS_BACKGROUND_TEXTURE, x, y, 0, 0, width, height, 64, 64);
        context.setShaderColor(1.0f, 1.0f, 1.0f, 1.0f);
